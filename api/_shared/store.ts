...
function dbRowToWorkflowRow(row: Record<string, unknown>): WorkflowRow {
  return {
    ...
    normalizedRow: objectValue(row['normalized_row']) as unknown as WorkflowRow['normalizedRow'],
...
function dbRuleToDefinition(row: Record<string, unknown>, variants: Record<string, unknown>[]): RuleDefinition {
  return {
    ...
    variants: variants.map((variant) => ({
      ...
      source: objectValue(variant['source']) as unknown as RuleDefinition['variants'][number]['source']
    })),
    createdAt: toIso(row['created_at']),
    updatedAt: toIso(row['updated_at'])
  };
...
function dbResultToRowExecutionResult(row: Record<string, unknown>): RowExecutionResult {
  return {
    ...
    beforeState: objectValue(row['before_state']) as unknown as WorkflowRow,
    afterState: objectValue(row['after_state']) as unknown as WorkflowRow,
